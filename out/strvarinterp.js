...
function Interpolate(str) {
    ...
    if (idx1 >= 0) {
        ...
        if (idx2 >= 0) {
            const from = (_) => _.then((ret) => (ret === undefined) ? Promise.reject() : Interpolate(str.slice(0, idx1) + ret + str.slice(idx2 + 1)), (err) => Promise.reject(err));
    ...
    return Promise.resolve((str.indexOf('${') < 0) ? str : Interpolate(str));
}
