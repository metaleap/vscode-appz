...
class Gen extends gen_syn.Gen {
    gen(prep) {
        ...
        if (this.isDemos) {
            ...
            node_fs.writeFileSync(outfilepath, "// Auto-merged-and-`sed`ed from `./proper-cmd/go-vsc-appz-demo` .go files to have a readily `go run`-able (even outside GOPATH) demo proggie that always imports our extension-local `../../libs/go` package\n" +
    ...
    emitStruct(it) {
        this.emitDocs(it)
            ...
            this.emitDocs(f).ln(() => this.s(f.Name, " ").emitTypeRef(f.Type).when(f.Json, () => this.s(" `json:\"")
